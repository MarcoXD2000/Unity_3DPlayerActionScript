...
public class PlayerController : MonoBehaviour
{
    ...
    //水平移動処理
    public void PlayerMove(Vector2 _moveDir)
    {
        //入力した移動方向
    ...
    //キャラ回転処理
    public void PlayerRotate(Vector2 _moveDir)
    {
    ...
    //    Debug.Log(rotateDir.x);

    //    //回転の量を追加
    //    eulerRotation.y += (float)((Math.Atan2(moveDir.x, moveDir.y)) * 180 / Math.PI);
    //    Quaternion deltaRotation = Quaternion.Euler(eulerRotation);

    //    //回転
    //    rotateTarget = deltaRotation;
    //    mRigid.rotation = Quaternion.RotateTowards(mRigid.rotation, rotateTarget, rotateSpeed * Time.deltaTime);
    //}
